  ...
  try {
    ...
    const [timeSlotsRes, classroomsRes, teachersRes, unavailabilityRes, assignmentsRes] =
      await Promise.all([
        ...
        pool.query(
          `SELECT classroom_id, classroom_name, grade_level
           FROM classrooms WHERE school_id = $1 ORDER BY classroom_name`,
          [school_id]
        ),
        pool.query(
    ...
    // Önce mevcut programı sil
    await client.query(
      'DELETE FROM schedules WHERE school_id = $1',
      [school_id]
    );
  ...
  try {
    ...
    let query = `
      SELECT
        ...
        cl.classroom_name, cl.grade_level,
...
export const updateScheduleEntry = async (req, res) => {
  try {
    ...
    // Aynı time_slot'ta bu öğretmen başka yerde var mı?
    if (teacher_id) {
